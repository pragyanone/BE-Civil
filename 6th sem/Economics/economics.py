...
def senstivity():
	...
	values = [float(a) for a in input('Enter the Investment, Annual Revenue, Annual Expense, number of time periods, interest rate, and salvage value separated by commas:\n').split(',')]
	values = [str(item) for item in values]
	for index, item in enumerate(float(values)):
		for delta in [-.4, -.3, -.2, -.1, 0, .1, .2, .3, .4]:
			command = ', '.join(values[:index]) + f', {item*(1+delta)}, ' + ', '.join(values[index+1:])
